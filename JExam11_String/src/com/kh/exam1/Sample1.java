...
public class Sample1 {
	...
	public static void main(String[] args) {
		...
		System.out.println("s.equals(\"String 관련 메서드\") -> " + b2);
		...
		// .comareTo(String anotherString)
		// 문자열과 anotherString의 문자열을 비교하여 사전순으로
		// 기존 문자열이 앞이면 음수, 뒤면 양수값을 반환
		String comp = new String("ABCD");
		int diff1 = comp.compareTo("CCDE");
		int diff2 = comp.compareTo("AACD");
		System.out.println("comp.compareTo(\"CCDE\") -> " + diff1);
		...
		String rep = new String("문자열 변경 테스트 테스트");
		...
		System.out.println("str2.toLowerCase() -> " + upStrRes);
		...
		// .trim();
		// 문자열에서 공백을 제거하여 반환(단, 문자열의 앞/뒤의 공백만 제거), 반환타입(String)
		// 사용자 입력에서 사용자가 불필요한 공백을 입력하는 경우 제거할 수 있다.
		...
		// String formatStr = String.format("%1$s-%2$s-%3$s", date);
