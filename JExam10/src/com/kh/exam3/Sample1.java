...
public class Sample1 {
	...
	public static void method1(Square s) {
		
	}
	
	public static void method2(Triangle t) {
		
	}
	
	// 매개변수에서 활용한 다형성
	public static void method3(Shape s) {
	...
	public static void main(String[] args) {
		...
		//			슈퍼객체에 정의된 멤버만 참조 가능.(자식에 새롭게 정의된 메서드나 필드에 접근 안됨)
		// 다운 캐스팅 : 슈퍼객체를 통해서 서브객체를 참조할 때 슈퍼객체가 가지고 있지 않은 멤버에
		//			  접근하기 위해서 참조형을 서브객체로 변환.(자동 변환 안됨)
		Shape p1 = new Square(10, 20);
		System.out.println(((Square)p1).name + " 넓이 : " + p1.getArea());
		
		Triangle p2 = new Triangle(10, 20);
		...
		for(int i = 0; i < shape.length; i++) {
			w = rand.nextInt(90) + 10;	h = rand.nextInt(90) + 10;
			if(i % 2 == 0) {
		...
		for(int i = 0; i < shape.length; i++) {
			System.out.println("너비 : " + shape[i].getWidth() + " | " + "높이 : " + shape[i].getHegith());
